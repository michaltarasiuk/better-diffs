      ...
      <Popover style={{width: 280}} {...popoverProps}>
        <div style={{display: 'flex', gap: 8, alignItems: 'center'}}>
          ...
          <div style={{minWidth: 0}}>
            <div style={{fontWeight: 600, fontSize: 'var(--font-size)'}}>
              {name}
            </div>
            <div style={{fontSize: 'var(--font-size-sm)'}}>@{handle}</div>
          </div>
          <Button style={{marginLeft: 'auto'}} variant="secondary">
            Follow
          </Button>
        </div>
    ...
    <PreviewTrigger>
      ...
      <Popover className="w-[280px] p-2" {...popoverProps}>
        <div className="flex items-center gap-2">
          <img alt="" src={avatar} className="size-10 shrink-0 rounded-full" />
          ...
          <Button
            className="ml-auto"
            variant="secondary"
            onPress={() => alert(`Followed @${handle}`)}
          >
            Follow
          </Button>
        </div>
        <div className="mt-3 text-sm">{bio}</div>
      </Popover>
    </PreviewTrigger>
  );
}
  ...
  return (
    ...
    <PreviewTrigger {...props}>
      ...
      <Popover style={{width: 280}}>
        ...
        <Button style={{marginTop: 12}} variant="secondary">
          View issue
        </Button>
      </Popover>
    </PreviewTrigger>
...
`PreviewTrigger` works with any focusable React Aria component (e.g. [Link](Link.md), [Button](Button.md), etc.). Use the `<Focusable>` component to wrap a custom trigger element such as a third party component or DOM element.
...
| -------------- | ---------------------------- | ---------- | --------------------------------------------------------- |
| `children`     | `React.ReactNode`            | —          | The trigger and Popover that make up the preview trigger. |
| `closeDelay`   | `number                      | undefined` | 200                                                       | The delay time in milliseconds before the preview closes.             |
| `defaultOpen`  | `boolean                     | undefined` | —                                                         | Whether the overlay is open by default (uncontrolled).                |
| `delay`        | `number                      | undefined` | 600                                                       | The delay time in milliseconds before the preview opens.              |
