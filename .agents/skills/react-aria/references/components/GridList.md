...
.react-aria-GridList {
  ...
  &[data-layout='grid']:not(:has(.react-aria-GridListSection)):not(
      [data-empty]
    ) {
    display: grid;
    ...
    &:not([data-orientation='horizontal']) {
      grid-template-columns: repeat(
        auto-fit,
        minmax(100px, var(--grid-item-size))
      );
    }
  }
  ...
  return (
    <AriaGridList
      ...
      className={composeTailwindRenderProps(
        props.className,
        isHorizontal
          ? 'relative flex w-full max-w-[500px] flex-row flex-nowrap overflow-x-auto rounded-lg border border-neutral-300 bg-white font-sans empty:flex empty:items-center empty:justify-center empty:text-sm empty:italic dark:border-neutral-700 dark:bg-neutral-900'
