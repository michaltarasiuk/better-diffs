  ...
  return (
    <AriaColumn
      ...
      className={composeTailwindRenderProps(
        props.className,
        'box-border h-1 cursor-default text-start text-sm font-semibold text-neutral-700 focus-within:z-20 dark:text-neutral-300 [&:hover]:z-20',
      )}
    >
      {composeRenderProps(
        props.children,
...
export function TableFooter<T>(props: TableFooterProps<T>) {
  return (
    <AriaTableFooter
      {...props}
      className="bg-neutral-200 font-bold dark:bg-neutral-700"
    />
  );
}
...
function FileTable() {
  let [showColumns, setShowColumns] = useState([
    'title',
    'status',
    'paymentMethod',
    'price',
...
export default function ResizableTable() {
  let columnWidths = useSyncExternalStore(
    subscribe,
    getColumnWidths,
    getInitialWidths,
  );
