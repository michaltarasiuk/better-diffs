...
>
  ...
  >
    {(item) => (
      <ListBoxItem style={{height: '100%', minHeight: 0}}>
        {item.name}
      </ListBoxItem>
    )}
  </ListBox>
    ...
    {(item) => (
      <GridListItem textValue={item.title}>
        <img
          src={item.image}
          alt=""
          style={{aspectRatio: item.aspectRatio, maxWidth: 'none'}}
        />
        <Text>{item.title}</Text>
...
function ExpandableItem({item}) {
  ...
  return (
    <div className="expandable-item">
      ...
      <Text slot="description" style={{display: expanded ? 'block' : 'none'}}>
...
>
  ...
  >
    {(item) => (
      <GridListItem textValue={item.name} style={{padding: 0}}>
